...
export default function Modal({ img, title, contents, btnList }: ModalProps) {
  return (
    <div className="w-full min-h-full z-20 fixed top-0 left-0 bg-[#333333]/50 flex justify-center items-center">
      <div className="bg-white w-64 gap-3 min-h-36 px-7 py-5 flex flex-col justify-center items-center overflow-hidden rounded-[30px] shadow-[0px_2px_15px_0px_rgba(111,111,111,0.20)]">
        ...
        <div className="flex justify-center items-center px-4 gap-2 pb-1">
          {btnList.map((item) => (
            <Button
              key={item.label}
              label={item.label}
